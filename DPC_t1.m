function [ ret ] = DPC_t1( A,KK ,percent,d,l)%,class_num,k)
...
%     plot([1:KK],gamma_sorted(1:KK),'*')
%   plot(gamma_sorted(1:21),'*')
% hold on
% 
%    plot([KK+1:ND],gamma_sorted(KK+1:end),'*')
%    hold off
%    figure
%     plot(rho(gamma_order(1:KK)),delta(gamma_order(1:KK)),'*');
%      hold on
%     plot(rho(gamma_order(KK+1:end)),delta(gamma_order(KK+1:end)),'*');
%     hold off
...
for i=1:ND-l+1 
    NNND=[1:1:ND];
   r(i,:)=polyfit(NNND(1:l),gamma_sorted(i:i+l-1),1);
 end
%     std_val=std(gamma_sorted);
%     class_num=length(find(gamma_sorted>3*std_val));
    r1=abs(r);
% class_num=length(find(r1(:,1)>=0.0005));
class_num=length(find(r1(:,1)>=d));
if class_num<KK
    class_num=KK;
end
BBB=[];
...
for i=1:class_num
    ...
    BBB=[BBB;A(gamma_order(i),:)];
end
%  bbb=BBB(2:class_num+1,:);

%     class_num=KK;
%     bbbb=linkage(BBB,'single');
     bbbb=linkage(BBB,'average');
BBBB=cluster(bbbb,KK);
